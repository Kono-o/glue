...
use crate::renderer::gl::{GL, GLError};
use crate::renderer::handles::{DrawMode, Mesh3D, Shader};
use crate::{
   CamProj, Camera, DataType, Image, Mesh2D, Mesh2DFile, Mesh3DFile, RGBA, ShaderFile, Size2D,
   Texture, Transform2D, Transform3D, Workers,
};
use cgmath::ortho;
use std::ops::Deref;
...
impl Renderer {
   ...
   pub fn add_compute_shader(
      ...
      shdr: ShaderFile,
      workers: Workers,
   ) -> Result<Shader, RendError> {
      self.add_shader_of_type(shdr, true, workers)
   }
   pub fn add_shader(&self, shdr: ShaderFile) -> Result<Shader, RendError> {
      self.add_shader_of_type(shdr, false, Workers::empty())
   }

   fn add_shader_of_type(
      &self,
      shdr: ShaderFile,
      is_compute: bool,
      workers: Workers,
   ) -> Result<Shader, RendError> {
      let (src1, src2) = match shdr {
         ShaderFile::Pipe { v_src, f_src } => (v_src, Some(f_src)),
         ShaderFile::Comp(src) => (src, None),
      };

      let id = match self.gl.create_src_program(&src1, &src2) {
         Ok(id) => id,
         Err(e) => {
   ...
   pub fn add_texture(&self, txtr: Image) -> Texture {
      let id = self.gl.create_texture(&txtr);
   ...
   pub fn add_mesh3d(&self, nmesh: Mesh3DFile) -> Mesh3D {
   ...
   pub fn remove_mesh3d(&self, mesh: Mesh3D) {
      drop(mesh)
   }

   pub fn add_mesh2d(&self, nmesh: Mesh2DFile) -> Mesh2D {
   ...
   pub fn remove_mesh2d(&self, mesh: Mesh2D) {
      drop(mesh)
   }

   pub fn render3d(&self, mesh: &Mesh3D) {
      if !mesh.is_visible() {
         return;
      }
      ...
      self
         .gl
         .set_uni_m4_f32(s, "uView", self.cam.transform.view_matrix());
      self
         .gl
         .set_uni_m4_f32(s, "uProj", self.cam.transform.proj_matrix());
      self.gl.set_uni_m4_f32(s, "uTfm", tfm);
   ...
   pub fn render2d(&self, mesh: &Mesh2D) {
      ...
      self.gl.bind_shader(s);
      self.gl.set_uni_m4_f32(s, "uProj", proj);
      self.gl.set_uni_m4_f32(s, "uTfm", tfm);
   ...
   pub fn compute(&self, shader: &Shader) {
      let s = shader.id;
      self.gl.bind_shader(s);

      self.bind_textures(shader.is_compute, &shader.tex_ids);
      self.gl.dispatch(shader.workers.groups())
   }

   fn bind_textures(&self, is_compute: bool, tex_ids: &Vec<Option<u32>>) {
      for (slot, tex_id) in tex_ids.iter().enumerate() {
         match tex_id {
            None => {}
            Some(id) => match is_compute {
               false => self.gl.bind_texture_sampler_at(*id, slot as u32),
               true => self.gl.bind_image_texture_at(*id, slot as u32),
            },
         }
      }
   }
